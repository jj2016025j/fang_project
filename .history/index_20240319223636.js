...
//座位訂單編輯
app.get('/shop/tableOrder', async(req, res) => {
...
//設定訂單品項(手機端)
app.post('/api/order/foods/:orderId', async(req, res) => {
    let formData = req.body;
    formData.orderId = req.params['orderId']
    try{
        await dataRep.updateOrderFoods(formData)
        res.status(200).send(true);
    }catch(e){
        res.status(400).json({
            error: e
