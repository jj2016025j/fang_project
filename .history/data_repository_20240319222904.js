    ...
    updateOrderFoods: async(foods) => {
        return new Promise((resolve, reject) => {
            pool.getConnection(async(err, connection) => {
                ...
                var values = [];
                foods.forEach((food) => {
                    var foodData = foods.find((f) => f.id == food.id);
                    if(foodData){
                        valuesSet.push('(?,?,?,?,?)');
                        values.push(orderId, foodData.id, foodData.item, foodData.price, foodData.item * foodData.price);
                    }
                });
