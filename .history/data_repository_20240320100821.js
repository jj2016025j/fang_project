    ...
    deleteOrderFood: async(orderId, foodId) => {
        return new Promise((resolve, reject) => {
            pool.getConnection((err, connection) => {
                ...
                connection.query(`DELETE FROM orders_items oi LEFT JOIN table_orders o ON o.id = oi.order_id where o.order_status = 1 AND oi.order_id = ? AND oi.food_id = ?`, [orderId, foodId], (error, results) => {
                    connection.release();
                    if (error) {
                        reject(error);
                        return;
