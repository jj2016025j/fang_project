...
const foodStorage = multer.diskStorage({ // notice you are calling the multer.diskStorage() method here, not multer()
    destination: function(req, file, cb) {
        cb(null, './public/uploads/foods/')
    },
    filename: function(req, file, cb) {
        var ext = file.mimetype.split('/')[1];
        cb(null,  'food_' + Date.now() + '.' + file.originalname.split('.').pop())
    }
});
const foodsUpload = multer({foodStorage})
const fs = require('fs');
