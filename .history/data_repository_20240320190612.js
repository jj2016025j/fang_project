    ...
    getReport: ()=> {
        return new Promise((resolve, reject) => {
            pool.getConnection((err, connection) => {
                ...
                var dayTurnoverSql = `SELECT SUM('trade_amt') FROM table_orders WHERE DATE(payment_at) = CURDATE() AND order_status = 2`;
                var monthTurnoverSql = `SELECT SUM('trade_amt') FROM table_orders WHERE MONTH(payment_at) = MONTH(CURDATE()) AND YEAR(payment_at) = YEAR(CURDATE()) AND order_status = 2`;
