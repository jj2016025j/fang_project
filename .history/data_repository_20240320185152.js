...
const repository = {
     ...
     * 取得報表
     * 1.日營業額: dayTurnover
     * 2.月營業額: monthTurnover
     * 3.月銷售排行榜前五名: rankTop5
     * 4.今年每月份的營業額: monthTurnoverOfYear
     */
    getReport: ()=> {
        return new Promise((resolve, reject) => {
            pool.getConnection((err, connection) => {
                ...
                var dayTurnoverSql = `SELECT * FROM table_orders WHERE DATE(payment_at) = CURDATE() AND order_status = 2`;
                var monthTurnoverSql = `SELECT * FROM table_orders WHERE MONTH(payment_at) = MONTH(CURDATE()) AND YEAR(payment_at) = YEAR(CURDATE()) AND order_status = 2`;
                var rankTop5ThisMonthSql = `SELECT food_id, SUM(quantity) as total FROM orders_items oi LEFT JOIN table_orders o ON o.id = oi.order_id WHERE o.order_status = 2 AND YEAR(payment_at) = YEAR(CURDATE()) AND MONTH(payment_at) = MONTH(CURDATE()) GROUP BY food_id ORDER BY total DESC LIMIT 5`;
                var monthTurnoverOfYearSql = `SELECT MONTH(payment_at) as month, SUM(trade_amt) as price FROM table_orders WHERE YEAR(payment_at) = YEAR(CURDATE()) AND order_status = 2 GROUP BY MONTH(payment_at)`;
                ...
                connection.query(oneSql, (error, results) => {
                    ...
                    resolve({
                        dayTurnover: results[0],
                        monthTurnover: results[1],
                        rankTop5: results[2],
